...
- **`output_type` JSON-schema validation** — every classification is validated server-side against an enum contract before it leaves Libra OS. Your routing logic acts on `intent`; you never parse free text or handle a model that invents a category.
- **`violation_mode=repair`** — if the first reply doesn't conform, Libra OS re-prompts once with the schema in the system prompt instead of returning a 422. For a high-volume classifier that one bounded retry is cheaper than error handling on your side.
...
- **Few-shot from your own tickets.** Ingest labelled examples as a knowledge collection and add `knowledge_bindings=["ticket-examples"]` to the agent — Libra OS injects the nearest labelled examples for each incoming ticket automatically (see the [`document-qa`](../document-qa/) recipe for the ingest pattern).

## Prose companion

The Libra OS docs guide covers the decision ("should you use an LLM for routing?"), success criteria, and integration patterns (push webhook vs pull batch): **https://docs.meganova.ai → Libra OS → Use Cases → Ticket Routing**.
